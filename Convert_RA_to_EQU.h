...
double ipart(double xx) {
	double sgn;
	if (xx < 0) sgn = -1.0;
	else if (xx == 0) sgn = 0.0;
	else if (xx > 0) sgn = 1.0;
...
// Convert Right Ascension to Equatorial --------------------------------------------------------------------
void Convert_RA_to_EQU(double RA, double Dec, double Lat, double Long, double hour, double minute, double day) {
#ifdef MONITOR_PRINT_RA_TO_DEC
	Serial.print(millis(), DEC); Serial.print("\tRight ascension: "); Serial.print(RA,DEC); Serial.print(", Declination: "); Serial.println(Dec, DEC);
#endif
...
#ifdef MONITOR_PRINT_RA_TO_DEC
	Serial.print(millis(), DEC); Serial.print("Azimuth: "); Serial.print(az, DEC); Serial.print(", Elevation: "); Serial.println(alt,DEC);
#endif
}
//void earth() {
...
int main() {
	...
	Convert_RA_to_EQU(((alpha) * 15), (delta), 12.824278, 80.042885, hh, mm, d);
	return 0;
}
#pragma once
