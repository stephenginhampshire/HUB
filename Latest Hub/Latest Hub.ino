...
uint8_t this_device = Devices::Hub;
// Constants --------------------------------------------------------------------------------------
...
constexpr uint8_t Altitude_TX_pin = 18;     // Altitude Port TX Serial1
constexpr uint8_t Altitude_RX_pin = 19;     // Altitude Port RX Serial1
constexpr uint8_t Azimuth_TX_pin = 16;      // Azimuth Port TX  Serial2
constexpr uint8_t Azimuth_RX_pin = 17;      // Azimuth Port RX  Serial2
constexpr uint8_t Focuser_TX_pin = 14;      // Focuser Port TX  Serial3
constexpr uint8_t Focuser_RX_pin = 15;      // Focuser Port RX  Serial3
...
void serialEvent2() {
    ...
    //    Serial.print(Azimuth_inbuffer[Azimuth_inptr - 1]);
        //    console_print(Devices::Azimuth, true, true, "Char RXd", true);
}
void serialEvent3() {
...
void setup() {
    ...
    console_print(this_device, setup_print, true, F("Setup Commenced"), true);
    ...
    console_print(this_device, setup_print, true, F("Starting Ethernet Initialisation"), true);
    ...
    if (Ethernet.hardwareStatus() == EthernetNoHardware) {
        ...
        console_print(this_device, setup_print, true, F("\tEthernet Shield Found"), true);
    }
    if (Ethernet.linkStatus() == LinkOFF) {
        console_print(this_device, setup_print, true, F("\tEthernet cable not connected"), true);
    }
    else {
        console_print(this_device, setup_print, true, F("\tEthernet cable connected"), true);
    }
    //EthernetClient client = server.accept();
    Status.bit.Operator = 1;
    console_print(this_device, setup_print, true, F("Ethernet Initialisation Complete"), true);
    console_print(this_device, setup_print, true, F("Serial Port Initialisation"), true);
    pinMode(Azimuth_RX_pin, INPUT);
    if (digitalRead(Azimuth_RX_pin)) {
        console_print(this_device, setup_print, true, F("\tAzimuth Communication Line Connected"), true);
        Status.bit.Azimuth = 1;
    }
    else {
        console_print(this_device, setup_print, true, F("\tAzimuth Communication Line not Connected"), true);
        Status.bit.Azimuth = 0;
    }
    Azimuth_Port.begin(Azimuth_baud);					// initialise the Altitude serial port    
    Azimuth_Port.flush();                                          // clear the Altitude serial buffer
    pinMode(Altitude_RX_pin, INPUT);
    if (digitalRead(Altitude_RX_pin)) {
        console_print(this_device, setup_print, true, F("\tAltitude Communication Line Connected"), true);
        Status.bit.Altitude = 1;
    }
    else {
        console_print(this_device, setup_print, true, F("\tAltitude Communication Line not Connected"), true);
        Status.bit.Altitude = 0;
    }
    Altitude_Port.begin(Altitude_baud);					// initialise the Azimuth serial port
    Altitude_Port.flush();											// clear the Azimuth serial buffer
    pinMode(Focuser_RX_pin, INPUT);
    if (digitalRead(Focuser_RX_pin)) {
        console_print(this_device, setup_print, true, F("\tFocuser Communication Line Connected"), true);
        Status.bit.Focuser = 1;
    }
    else {
        console_print(this_device, setup_print, true, F("\tFocuser Communication Line not Connected"), true);
        Status.bit.Focuser = 0;
    }
    Focuser_Port.begin(Focuser_baud);					// initialise the Focuser serial port
    Focuser_Port.flush();											// clear the Focuser serial buffer
    console_print(this_device, setup_print, true, F("Serial Port Initialisation Complete"), true);
    console_print(this_device, setup_print, true, F("Enabling WatchDog Timer"), true);
    ...
    if (getWdtTimeoutMs()) {
        ...
        console_print(this_device, setup_print, true, Display_Buffer, true);
    }
    else {
        console_print(this_device, setup_print, true, F("\tWatchdog Timer Initialisation failure"), true);
    }
    console_print(this_device, setup_print, true, F("Watchdog Timer Initialisation Complete"), true);
    Led_Control(RUN_Active_led_pin, ON);
    console_print(this_device, setup_print, true, F("Setup Complete"), true);
    console_print(this_device, setup_print, true, F("Starting Main Loop"), true);
} // end setup
// Main -------------------------------------------------------------------------------------------
void loop() {
...
bool Check_Operator_Packet_Received(void) {
    if (server.available()) {
        ...
        console_print(this_device, comms_print, true, "Client connected", true);
    }
    while (client && client.connected() && client.available() > 0) {
        ...
        if (thisbyte == (uint8_t)Control_Characters::STX) {                 // look for start character
            ...
            if (thisbyte == (uint8_t)Control_Characters::EOT) {                                 // characters was not an STX check for ETX
                ...
                console_print(this_device, comms_print, true, "\tIncoming Packet from Operator:", true);
    ...
    while (Altitude_outptr != Altitude_inptr) {                                         // check Altitude serial buffer for data
        ...
        if (thisbyte == (uint8_t)Control_Characters::STX) {                             // look for the SOH
            ...
            if (thisbyte == (uint8_t)Control_Characters::ETX) {                         // characters was not an SOH check for ETX
                ...
                console_print(this_device, comms_print, true, "\tIncoming Packet from Altitude:", true);
...
bool Check_Azimuth_Packet_Received(void) {
    while (Azimuth_outptr != Azimuth_inptr) { // check Azimuth serial buffer for data
        console_print(Devices::Azimuth, true, true, "Check true", true);
        ...
        if (thisbyte == (uint8_t)Control_Characters::STX) {                                 // look for the SOH
            ...
            if (thisbyte == (uint8_t)Control_Characters::ETX) {                             // characters was not an STX check for ETX
                ...
                console_print(this_device, comms_print, true, "\tIncoming Packet from Azimuth:", true);
    ...
    while (Focuser_outptr != Focuser_inptr) {                                           // check Focuser serial buffer for data
        ...
        if (thisbyte == (uint8_t)Control_Characters::STX) {                             // look for the SOH
            ...
            if (thisbyte == (uint8_t)Control_Characters::ETX) {                                            // characters was not an STX check for ETX
                ...
                console_print(this_device, comms_print, true, "\tIncoming Packet from Focuser:", true);
...
bool Process_Packet_Received_from_Operator() {                                   // Process a packet from the Operator  
    console_print(this_device, comms_print, true, F("Processing Packet Received from Operator"), true);
    switch (Incoming_Packet_from_Operator.field.MessageTarget) {                 // switch on the target
    case Devices::Hub: {
        console_print(this_device, comms_print, true, F("Destination HUB: "), true);
        switch (Incoming_Packet_from_Operator.field.CommandNumber) {             // switch on the Command Number
        case (Commands::CMD_Reset): {
            console_print(this_device, comms_print, true, F("Reset Command Received from Operator"), true);
        ...
        case (Commands::Request_Firmware_Version): {
            console_print(this_device, comms_print, true, F("Firmware Version Get Received from Operator"), true);
        ...
        case (Commands::Statistics): {
            console_print(this_device, comms_print, true, F("Statistics Get Received from Operator"), true);
            if (Incoming_Packet_from_Operator.field.PacketType == GET) {
                Prepare_and_Send_Reply_to_Operator(Operator,
                    Statistics,
                    ALT_Packet_Received_Count,
    ...
    case Devices::Altitude: {                                                     // send the packet to the ALT
        console_print(this_device, comms_print, true, F("Packet Destination Altitude"), true);
        Transmit_Packet_to_Target(Incoming_Packet_from_Operator);
        break;
    }
    case Devices::Azimuth: {
        console_print(this_device, comms_print, true, F("Packet Destination Azimuth"), true);
        Transmit_Packet_to_Target(Incoming_Packet_from_Operator);
        break;
    }
    case Devices::Focuser: {
        console_print(this_device, comms_print, true, F("Packet Destination Focuser"), true);
        Transmit_Packet_to_Target(Incoming_Packet_from_Operator);
        break;
    }
    }
...
void Transmit_to_Operator(PacketUnion data) {
    if (Status.bit.Operator) {                           // check if Operator is available
        while (!server.availableForWrite()) {
            console_print(this_device, comms_print, true, F("Waiting for server available"), true);
            delay(10);
        }
        console_print(this_device, comms_print, true, F("Sending Packet to Operator"), true);
...
void Transmit_Packet_to_Target(PacketUnion data) {
    ...
    case Devices::Azimuth: {
        ...
        console_print(this_device, comms_print, true, F("Packet sent to Azimuth"), true);
        AZI_Packet_Transmitted_Count++;
        break;
    }
    case Devices::Altitude: {
        ...
        console_print(this_device, comms_print, true, F("Packet sent to Altitude"), true);
        ALT_Packet_Transmitted_Count++;
        break;
    }
    case Devices::Focuser: {
        ...
        console_print(this_device, comms_print, true, F("Packet sent to Focuser"), true);
        FOC_Packet_Received_Count++;
        break;
    }
    }
...
void Wait_for_Reset(const __FlashStringHelper* message) {
    int trys = 0;
    console_print(this_device, comms_print, true, F("Failure: "), true);
    Serial.print(message);
    console_print(this_device, comms_print, true, F(", Press Reset"), true);
    do {
        delay(500);
