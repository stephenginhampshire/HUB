...
static volatile byte twi_sendStop;			// should the transaction end with a stop
static volatile byte twi_inRepStart;			// in the middle of a repeated start
...
static void (*twi_onSlaveReceive)(byte*, int);

static byte twi_masterBuffer[TWI_BUFFER_LENGTH];
static volatile byte twi_masterBufferIndex;
static volatile byte twi_masterBufferLength;

static byte twi_txBuffer[TWI_BUFFER_LENGTH];
static volatile byte twi_txBufferIndex;
static volatile byte twi_txBufferLength;

static byte twi_rxBuffer[TWI_BUFFER_LENGTH];
static volatile byte twi_rxBufferIndex;

static volatile byte twi_error;
 ...
 * Output   none
 */
void twi_setAddress(byte address)
{
 ...
 */
byte twi_readFrom(byte address, byte* data, byte length, byte sendStop)
{
    byte i;
 ...
 *          5 .. timeout
 */
byte twi_writeTo(byte address, byte* data, byte length, byte wait, byte sendStop)
{
    byte i;
 ...
 *          0 ok
 */
byte twi_transmit(const byte* data, byte length)
{
    byte i;
 ...
 * Output   none
 */
void twi_attachSlaveRxEvent(void (*function)(byte*, int))
{
...
void twi_handleTimeout(bool reset) {
    ...
    if (reset) {
        ...
        byte previous_TWBR = TWBR;
        byte previous_TWAR = TWAR;
